...
void UNWGameInstance::JoinOnlineGame(FName SessionName) {
    ULocalPlayer* const Player = GetFirstGamePlayer();
    ...
    if (SessionSearch->SearchResults.Num() > 0) {
        for (int32 i = 0; i < SessionSearch->SearchResults.Num(); i++) {
            ...
            if (SessionSearch->SearchResults[i].Session.OwningUserId != Player->GetPreferredUniqueNetId()) {
                ...
                // here
                JoinAtSession(Player->GetPreferredUniqueNetId(), SessionName, SearchResult);
                break;
            }
        }
    }
...
void UNWGameInstance::DestroySessionAndLeaveGame(FName SessionName) {
    ...
    if (OnlineSub) {
        ...
        if (Sessions.IsValid() && UserId.IsValid()) {
            ...
            */
            ...
            SessionSettings->Set(SETTING_MAPNAME, FString("test_map"), EOnlineDataAdvertisementType::ViaOnlineService);
...
void UNWGameInstance::OnStartOnlineGameComplete(FName SessionName, bool bWasSuccessful) {
    ...
    if (bWasSuccessful) {
        UGameplayStatics::OpenLevel(GetWorld(), _MapName, true, "listen");
    }
}

void UNWGameInstance::FindSessions(TSharedPtr<const FUniqueNetId> UserId, bool bIsLAN,
                                   bool bIsPresence) {
        ...
        if (Sessions.IsValid()) {
            ...
            if (SessionSearch->SearchResults.Num() > 0) {
                ...
                for (int32 SearchIdx = 0; SearchIdx < SessionSearch->SearchResults.Num(); SearchIdx++) {
                    ...
                    FString name = SessionSearch->SearchResults[SearchIdx].Session.OwningUserName;
...
/************************************** VOICE ************************************************/
//void UNWGameInstance::RegisterVoice() {
//    IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
//    ULocalPlayer* const Player = GetFirstGamePlayer();
//
//    if (OnlineSub && Player) {
//        IOnlineVoice* VoiceItf = OnlineSub->GetVoiceInterface().Get();
//        VoiceItf->RegisterRemoteTalker(*Player->GetPreferredUniqueNetId());
//    }
//}